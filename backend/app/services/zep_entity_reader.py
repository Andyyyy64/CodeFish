...
class ZepEntityReader:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"Zep {operation_name} {max_retries} 回試行後も失敗: {str(e)}")
    ...
    def get_all_nodes(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"合計 {len(nodes_data)} 個のノードを取得")
        return nodes_data
    ...
    def get_all_edges(self, graph_id: str) -> List[Dict[str, Any]]:
        ...
        logger.info(f"合計 {len(edges_data)} 本のエッジを取得")
        return edges_data
    ...
    def get_node_edges(self, node_uuid: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            edges = self._call_with_retry(
                ...
                operation_name=f"ノードエッジ取得(node={node_uuid[:8]}...)"
            )
            ...
            logger.warning(f"ノード {node_uuid} のエッジ取得失敗: {str(e)}")
            return []
    ...
    def filter_defined_entities(
        ...
        """
        logger.info(f"グラフ {graph_id} のエンティティをフィルタリング中...")
        ...
        logger.info(f"フィルタリング完了: 総ノード数 {total_count}, 条件一致 {len(filtered_entities)}, "
                   f"エンティティタイプ: {entity_types_found}")
    ...
    def get_entity_with_context(
        ...
        try:
            ...
            node = self._call_with_retry(
                ...
                operation_name=f"ノード詳細取得(uuid={entity_uuid[:8]}...)"
            )
            ...
            logger.error(f"エンティティ {entity_uuid} の取得失敗: {str(e)}")
            return None
