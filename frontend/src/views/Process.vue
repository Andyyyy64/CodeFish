<template>
  <div class="process-page">
    <!-- 顶部导航栏 -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">CODEFISH</div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">データがまもなく表示されます...</p>
          </div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  ドキュメントアップロード後、LLMがドキュメント内容を分析し、シミュレーションに適したオントロジー構造（エンティティタイプ＋関係タイプ）を自動生成
                </div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                ...
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} 他の関係...
                  </div>
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">API説明</div>
                <div class="detail-content">
                  生成されたオントロジーに基づき、ドキュメントを分割してZep APIで知識グラフを構築し、エンティティと関係を抽出
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                ...
                <div class="phase-api">次のステップに進む準備ができました</div>
              </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'アップロード待ちのファイルがありません。トップページに戻ってください'
    loading.value = false
    return
  }
  ...
  try {
    ...
    ontologyProgress.value = { message: 'ファイルをアップロードし、ドキュメントを分析中...' }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'プロジェクトの読み込みに失敗'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'プロジェクトの読み込みに失敗: ' + (err.message || '不明なエラー')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'グラフ構築タスクを開始しました...'
      ...
      error.value = response.error || 'グラフ構築の開始に失敗'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'グラフ構築の開始に失敗: ' + (err.message || '不明なエラー')
    buildProgress.value = null
  }
}
