            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">生成中...{{ section.title }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : '世界中の任意の個体と対話' }}</span>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">レポート生成エージェントの高速対話版。4種類の専門ツールを呼び出し可能、CodeFishの完全な記憶を保持</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">現実世界のシードデータとシミュレーション環境の状態を整合し、Global/Local Memoryメカニズムと組み合わせて時空を超えた深層帰属分析を提供</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch パノラマ追跡</div>
                    <div class="tool-desc">グラフ構造に基づく幅優先探索アルゴリズムでイベント伝播パスを再構築し、全情報フローのトポロジー構造を捕捉</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">GraphRAGベースの即時クエリインターフェース。インデックス効率を最適化し、具体的なノード属性と離散的な事実を素早く抽出</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    ...
                    <div class="tool-desc">自律型インタビュー。シミュレーション世界の個体と並行して複数ラウンドの対話を行い、非構造化の意見データと心理状態を収集</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">紹介</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Report Agentと対話し、レポート内容を深く理解する' : 'シミュレーション個体と対話し、彼らの視点を理解する' }}
              </p>
            </div>
            <div 
...
const sendToAgent = async (message) => {
  ...
  if (res.success && res.data) {
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.username} が回答`)
    } else {
...
const submitSurvey = async () => {
  ...
  addLog(`${selectedAgents.value.size} 件のAgentにアンケートを送信中...`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${surveyResults.value.length} 件の回答を受信`)
    } else {
      throw new Error(res.error || 'リクエスト失敗')
    }
  } catch (err) {
    addLog(`アンケート送信失敗: ${err.message}`)
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    addLog(`レポートデータ読み込み: ${props.reportId}`)
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog('レポートデータ読み込み完了')
    }
  } catch (err) {
    addLog(`レポートログ読み込み失敗: ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${profiles.value.length} 体のシミュレーションAgentを読み込み`)
    }
  } catch (err) {
    addLog(`シミュレーションAgent読み込み失敗: ${err.message}`)
  }
}
